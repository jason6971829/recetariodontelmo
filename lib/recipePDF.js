...
export async function generateRecipePDF(recipe) {
  ...
  if (recipe.ingredients?.length > 0) {
    ...
    recipe.ingredients.forEach((ing, i) => textLine("*  " + ingredientToString(ing), i % 2 === 0));
    y += 5;
  }
...
async function drawPizzaCanvas(portions, sectionData) {
  ...
  if (baseInfo) {
    ...
    // scale = cuánto hay que escalar para que la dimensión mayor llene el canvas
    const coverScale = Math.max(SZ / img.naturalWidth, SZ / img.naturalHeight) * 1.15;
    const dw = img.naturalWidth  * coverScale;
    const dh = img.naturalHeight * coverScale;
    // Para "center 60%": dx = -(exceso_x * 0.5), dy = -(exceso_y * 0.6)
    const dx = -(dw - SZ) * 0.5;
    const dy = -(dh - SZ) * 0.6;
    ctx.save();
    ctx.beginPath();
...
export async function generatePizzaPDF({ size, cfg, liveResults }) {
  ...
  liveResults.forEach((sec, idx) => {
    ...
    sec.ingredients.forEach((ing, j) => {
      const wrapped = doc.splitTextToSize("*  "+ingredientToString(ing), CW-6);
