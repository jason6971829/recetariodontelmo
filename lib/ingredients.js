...
// Para guardar/exportar como string sencillo (Excel viejo, PDF, API)
// CRITICO: el "|" debe aparecer SOLO una vez, antes del codigo.
// Si el texto del ingrediente contiene "|" (ej: "S. Aritos | 65gr"), lo cambia
// a "*" para que gabycontrol pueda parsear sin ambiguedad.
export function ingredientToString(ing) {
  const { code, text } = normalizeIngredient(ing);
  const safeText = String(text).replace(/\|/g, "*").trim();
  return code ? `${safeText} | ${code}` : safeText;
}
