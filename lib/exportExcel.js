...
function buildIndex(recipes, brandName) {
  const ws = {};
...
function buildCategorySheet(catRecipes) {
  const ws = {};
  let row = 1;
...
export function exportToExcel(recipes, brandName = "Recetario") {
  const wb = XLSX.utils.book_new();

  // Hoja 1: Índice
  const wsIndex = buildIndex(recipes, brandName);
  ...
  Object.entries(byCategory).forEach(([cat, recs]) => {
    const ws = buildCategorySheet(recs);
