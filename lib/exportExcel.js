...
function formatIngredients(ingredients) {
  ...
  return ingredients
    .map(ing => {
      // Soporta formato viejo (quantity/unit/name) y nuevo ({code, text})
      if (ing && typeof ing === "object" && (ing.quantity || ing.unit)) {
      ...
      return n.code ? `[${n.code}] ${n.text}` : n.text;
    })
    .join("\n");
}
...
function buildCategorySheet(catRecipes, XLSX) {
  ...
  catRecipes.forEach((r, idx) => {
    ...
    if (r.ingredients && r.ingredients.length > 0) {
      // Encabezados de ingredientes (con columna CODIGO al inicio)
      ws[`A${row}`] = { v: "CODIGO",      t:"s", s: hdrStyle("D4A574", "1B3A5C") };
      ws[`B${row}`] = { v: "CANTIDAD",    t:"s", s: hdrStyle("D4A574", "1B3A5C") };
      ws[`C${row}`] = { v: "UNIDAD",      t:"s", s: hdrStyle("D4A574", "1B3A5C") };
      ws[`D${row}`] = { v: "INGREDIENTE", t:"s", s: hdrStyle("D4A574", "1B3A5C") };
      row++;
      r.ingredients.forEach((ing, i) => {
        const bg = i % 2 === 0 ? "FFFFFF" : "FDF8F2";
        // Soporta formato viejo (quantity/unit/name) y nuevo ({code, text})
        const hasOldFormat = ing && typeof ing === "object" && (ing.quantity || ing.unit);
        const n = hasOldFormat ? null : normalizeIngredient(ing);
        ws[`A${row}`] = { v: hasOldFormat ? "" : str(n.code),                                 t:"s", s: cellStyle({ fill: bg, center:true }) };
        ws[`B${row}`] = { v: hasOldFormat ? str(ing.quantity) : "",                            t:"s", s: cellStyle({ fill: bg, center:true }) };
        ws[`C${row}`] = { v: hasOldFormat ? str(ing.unit) : "",                                t:"s", s: cellStyle({ fill: bg, center:true }) };
        ws[`D${row}`] = { v: hasOldFormat ? str(ing.name) : str(n.text),                       t:"s", s: cellStyle({ fill: bg }) };
        row++;
      });
    } else {
