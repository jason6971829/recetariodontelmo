...
export async function saveProfileConfig(config) {
  try {
    ...
    const { error } = await supabase.storage
      .from("recipe-images")
      .upload("profile/config.json", bytes, { upsert: true, cacheControl: "0", contentType: "application/json" });
...
export async function loadProfileConfig() {
  try {
    ...
    if (!res.ok) return null;
    return await res.json();
  } catch (err) { console.error("loadProfileConfig error:", err); return null; }
}
...
export async function saveAppConfig(config) {
  try {
    ...
    const { error } = await supabase.storage
      .from("recipe-images")
      .upload("app/config.json", bytes, { upsert: true, cacheControl: "0", contentType: "application/json" });
...
// Busca el ícono del logo directamente en Supabase Storage (brand/brand-icon.*)
// Funciona aunque app/config.json no exista todavía.
async function findBrandIconUrl() {
  for (const ext of ["png", "jpg", "jpeg", "webp", "gif", "svg"]) {
    const path = `brand/brand-icon.${ext}`;
    const { data: imgUrl } = supabase.storage.from("recipe-images").getPublicUrl(path);
    try {
      const check = await fetch(imgUrl.publicUrl + "?t=" + Date.now(), { method: "HEAD" });
      if (check.ok) return imgUrl.publicUrl;
    } catch {}
  }
  return null;
}
...
export async function loadAppConfig() {
  try {
    ...
    if (!res.ok) {
      const icon = await findBrandIconUrl();
    ...
    return config;
  } catch (err) { console.error("loadAppConfig error:", err); return null; }
}
