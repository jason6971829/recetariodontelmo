...
export async function saveUsers(users) {
  // Borrar todos y reinsertar (simple para pocos usuarios)
  const { error: delError } = await supabase.from("users").delete().neq("id", 0);
  if (delError) { console.error("saveUsers delete error:", delError); return false; }
  const { error: insError } = await supabase.from("users").upsert(users, { onConflict: "id" });
  if (insError) { console.error("saveUsers insert error:", insError); return false; }
  return true;
}
