...
export async function loadAppConfig() {
  try {
    ...
    if (!res.ok) return null;
    const config = await res.json();
    // Si no hay ícono guardado en el JSON, buscarlo en brand/brand-icon.*
    // (igual que hace loadWatermarkConfig con el logo de marca de agua)
    if (config.brand && !config.brand.icon) {
      for (const ext of ["png", "jpg", "jpeg", "webp", "gif", "svg"]) {
        ...
        try {
          const check = await fetch(imgUrl.publicUrl + "?t=" + Date.now(), { method: "HEAD" });
          if (check.ok) { config.brand.icon = imgUrl.publicUrl; break; }
        } catch {}
      }
    }
    return config;
